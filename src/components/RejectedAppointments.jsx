...
import { Calendar, Menu, User, CheckCircle , LogOut, XCircle } from "lucide-react";
...
export default function RejectedAppointments() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const res = await axios.get(`${backend_url}/api/appointments/rejected`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setAppointments(res.data.data || []);
      } catch (err) {
        console.error("Failed to fetch rejected appointments:", err.response?.data || err.message);
        setAppointments([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="flex h-screen bg-gradient-to-tr from-indigo-50 via-white to-indigo-100">
      ...
      >
        <div className="flex flex-col h-full">
         ...
         <button  onClick={() => navigate("/admin-dashboard")}
            ...
            <button
              className="flex items-center gap-3 py-3 px-4 rounded-2xl hover:bg-indigo-50 transition-colors"
              onClick={() => navigate("/admin/approved-appointments")}
            >
      ...
      <div className="flex flex-col flex-1 overflow-y-auto">
        ...
        <main className="p-8 max-w-7xl w-full mx-auto">
          {loading ? (
            <div>Loading rejected sessions...</div>
              ...
              {appointments.map((appt) => (
                ...
                >
                  <header className="flex justify-between items-start mb-4">
                    ...
                    <span
                      className="text-xs px-4 py-1 rounded-full font-semibold tracking-wider bg-red-200 text-red-900 select-none"
