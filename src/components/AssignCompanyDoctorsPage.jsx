...
export default function AssignCompanyDoctorsPage() {
  ...
  const fetchDoctors = async () => {
    ...
    try {
      ...
      // ✅ Fetch all doctors
      const allRes = await axios.get(`${backend_url}/api/companies/doctors/all`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const allDocs = allRes.data.data || [];

      // ✅ Fetch assigned doctors for this company
      const assignedRes = await axios.get(`${backend_url}/api/companies/${companyId}/doctors`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleAssignDoctor = async (doctorId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${backend_url}/api/companies/${companyId}/doctors`,
        { doctorId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleUnassignDoctor = async (doctorId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${backend_url}/api/companies/${companyId}/doctors/${doctorId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-blue-100 to-cyan-200">
      {/* Sidebar */}
      <div
      ...
      <div className="flex-1 flex flex-col py-12 px-4 sm:px-8 transition-all duration-500">
        ...
        <div className="max-w-3xl mx-auto w-full border border-indigo-300 rounded-3xl shadow-2xl bg-white/70 backdrop-blur-xl p-6 animate-fadein">
          ...
          {loading ? (
            ...
            <>
              ...
              <div>
                ...
                {allDoctors.length === 0 ? (
                  <p className="text-gray-500">No doctors available to assign.</p>
                ) : (
                  <ul className="flex flex-col gap-3">
                    {allDoctors.map((doc) => (
                      <li
